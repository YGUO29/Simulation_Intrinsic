function [cfg, det1, seeds] = MC1(config, exitangle, para)
...
%     cfg.seed=hex2dec('623F9A9E'); 
    ...
    cfg.nphoton=5e7;
    cfg.unitinmm = 0.005; % define units in mm
%     cfg.size = 300.*ones(1,3); % if unit = 0.05: 15mm cube 
...
switch cfg.config
    ...
    case 'without skull' % 1: gray; 2: white
        ...
        cfg.bounds = floor(1.5/cfg.unitinmm); 
...
%         cfg.bounds = [round(0.5/cfg.unitinmm), round((0.5+1.3)/cfg.unitinmm)]; 
%         if cfg.bounds(2) < cfg.size(3) % if simulation volume contains gray/white boundary
%             cfg.vol(:,:,cfg.bounds+1:bounds(2)) = 2; % gray matter
%             cfg.vol(:,:,cfg.bounds(2)+1:end) = 3; % the rest: white matter 
%         else
%             cfg.bounds = cfg.bounds(1);
%             cfg.vol(:,:,cfg.bounds+1:end) = 2;
%         end
%         cfg.vol=uint8(cfg.vol);
%         cfg.detpos=[cfg.size(1)/2, cfg.size(2)/2, cfg.bounds(1)-0.01, min(cfg.size(1)/sqrt(2), cfg.size(2)/sqrt(2))]; % [x y z radius]
    % ========== with skull ==========
    case 'with skull'
        cfg.vol = ones(cfg.size); % 15 mm volume
        cfg.bounds = [floor(0.7/cfg.unitinmm) floor(0.7/cfg.unitinmm)+floor(1.5/cfg.unitinmm)]; % skull, gray matter, white matter
        cfg.vol(:,:,cfg.bounds(1)+1:cfg.bounds(2))=2; % 0.7mm: skull
        cfg.vol(:,:,cfg.bounds(2)+1:end)=3; % the rest: white matter
        cfg.vol=uint8(cfg.vol);
...
%         cfg.bounds = [round(0.5/cfg.unitinmm) round((0.5+0.7)/cfg.unitinmm) round((0.5+0.7+1.3)/cfg.unitinmm) ]; %0.5mm air, 0.7mm skull, 1.3 gray matter
%         cfg.vol(:,:,cfg.bounds(1)+1:cfg.bounds(2)) = 1; % skull
%         if cfg.bounds(3) < cfg.size(3) % if simulation volume contains gray/white boundary
%             cfg.vol(:,:,cfg.bounds(2)+1:cfg.bounds(3)) = 2; % gray matter
%             cfg.vol(:,:,cfg.bounds(3)+1:end) = 3; % gray matter
%         else
%             cfg.vol(:,:,cfg.bounds(2)+1:end) = 2; % gray matter
%         end
%         cfg.vol=uint8(cfg.vol);
%         cfg.detpos=[cfg.size(1)/2, cfg.size(2)/2, cfg.bounds(1)-0.01, min(cfg.size(1)/sqrt(2), cfg.size(2)/sqrt(2))]; % [x y z radius]
    otherwise
...
%     end
    ...
    surface_profile = surface(:,199);
% ========== detected photons ==========
    ExitAngle           = vecnorm(det1{i}.v(:,1:2)'); % sin_theta (i.e. NA), v is a normal vector
    det1{i}.DetInd1     = find(det1{i}.p(:,3)<round(0.5/cfg.unitinmm) & det1{i}.v(:,3)<0);
