...
% for i = 1
    switch cfg.config
        case 'without skull'
            ...
            brain_mua(ind(i)) gray_matter_mus(ind(1)) gray_matter_g(ind(i)), 1.37   % medium 1: gray matter
            brain_mua(ind(i)) white_matter_mus(ind(1)) white_matter_g(ind(i)), 1.37];   % medium 2: white matter
        case 'with skull'
            cfg.prop=[0 0 1 1            % medium 0: the environment
...
% for i = 3:6
for i = 1:length(ind)
newcfg.seed=seeds{i}.data;
newcfg.detphotons=det1{i}.data;

cfg.prop=[0 0 1 1            % medium 0: the environment
skull_mua(ind(i)) skull_mus(ind(i)) 0.9337, 1.56 % medium 1: skull
brain_mua(ind(i)) gray_matter_mus(ind(2)) gray_matter_g(ind(i)), 1.37   % medium 2: gray matter
brain_mua(ind(i)) white_matter_mus(ind(2)) white_matter_g(ind(i)), 1.37];   % medium 3: white matter
