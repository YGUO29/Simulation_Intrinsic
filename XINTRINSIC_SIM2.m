...
% This function is for faster (mua) perturbation MC method by deviding the
% volume into small colums in the first place
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%    info=mcxlab('gpuinfo')
...
cfg.pertmethod = 'xcolumn'; %'xcolumn': change one column along x direction every time; 'r': change a circle
SimVol = 20;
switch cfg.config
    case 'without skull'
        if strcmp(cfg.pertmethod,'xcolumn')
            ...
            for i = 1:SimVol/2
                cfg.vol(cfg.size(1)/2+i,cfg.size(2)/2,1:bounds)=2*i+1;
                cfg.vol(cfg.size(1)/2+i,cfg.size(2)/2,bounds+1:end)=2*i+2;
            end
...
%             brain_mua(ind(i))*1.1 white_matter_mus(ind(i)) white_matter_g(ind(i)), 1.37];   % medium 4: white matter, pertubed
            nInd = SimVol+2;
            prop_temp = [brain_mua(ind(i)) gray_matter_mus(ind(i)) gray_matter_g(ind(i)), 1.37   % medium 1: gray matter
            brain_mua(ind(i)) white_matter_mus(ind(i)) white_matter_g(ind(i)), 1.37];   % medium 2: white matter
            cfg.prop = [0 0 1 1 % medium 0
                repmat(prop_temp,nInd/2,1)];
            else
            cfg.prop = [0 0 1 1            % medium 0: the environment
            brain_mua(ind(i)) gray_matter_mus(ind(i)) gray_matter_g(ind(i)), 1.37   % medium 1: gray matter
            brain_mua(ind(i)) white_matter_mus(ind(i)) white_matter_g(ind(i)), 1.37   % medium 2: white matter
